 ...
 *
 */
public class DragNDropSortListView extends ListView {
     ...
     * 
     * @attr name dslv:dragNdrop_enabled
     */
    ...
    public DragNDropSortListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs,
                    R.styleable.DragNDropSortListView, 0, 0);

            mItemHeightCollapsed = Math.max(1, a.getDimensionPixelSize(
                    R.styleable.DragNDropSortListView_drag_collapsed_height, 1));

            mTrackDragSort = a.getBoolean(
                    R.styleable.DragNDropSortListView_track_dragNdrop_sort, false);
            ...
            mFloatAlpha = a.getFloat(R.styleable.DragNDropSortListView_drag_float_alpha, mFloatAlpha);
            mCurrFloatAlpha = mFloatAlpha;

            mDragEnabled = a.getBoolean(R.styleable.DragNDropSortListView_dragNdrop_enabled, mDragEnabled);

            mSlideRegionFrac = Math.max(0.0f,
                    Math.min(1.0f, 1.0f - a.getFloat(
                            R.styleable.DragNDropSortListView_drag_slide_shuffle_speed,
                            0.75f)));

            mAnimate = mSlideRegionFrac > 0.0f;

            float frac = a.getFloat(
                    R.styleable.DragNDropSortListView_dragNdrop_scroll_start,
                    mDragUpScrollStartFrac);

            setDragScrollStart(frac);

            mMaxScrollSpeed = a.getFloat(
                    R.styleable.DragNDropSortListView_max_dragNdrop_scroll_speed,
                    mMaxScrollSpeed);

            removeAnimDuration = a.getInt(
                    R.styleable.DragNDropSortListView_drag_remove_animation_duration,
                    removeAnimDuration);

            dropAnimDuration = a.getInt(
                    R.styleable.DragNDropSortListView_dragNdrop_animation_duration,
                    dropAnimDuration);

            boolean useDefault = a.getBoolean(
                    R.styleable.DragNDropSortListView_drag_use_default_controller,
                    true);

            if (useDefault) {
                boolean removeEnabled = a.getBoolean(
                        R.styleable.DragNDropSortListView_drag_remove_enabled,
                        false);
                int removeMode = a.getInt(
                        R.styleable.DragNDropSortListView_drag_remove_mode,
                        DragSortController.FLING_REMOVE);
                boolean sortEnabled = a.getBoolean(
                        R.styleable.DragNDropSortListView_dragNsort_enabled,
                        true);
                int dragInitMode = a.getInt(
                        R.styleable.DragNDropSortListView_dragNdrop_start_mode,
                        DragSortController.ON_DOWN);
                int dragHandleId = a.getResourceId(
                        R.styleable.DragNDropSortListView_dragNdrop_handle_id,
                        0);
                int flingHandleId = a.getResourceId(
                        R.styleable.DragNDropSortListView_drag_fling_handle_id,
                        0);
                int clickRemoveId = a.getResourceId(
                        R.styleable.DragNDropSortListView_drag_click_remove_id,
                        0);
                int bgColor = a.getColor(
                        R.styleable.DragNDropSortListView_dragfloat_background_color,
                        Color.BLACK);
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView != null) {
                ...
                child = mAdapter.getView(position, oldChild, DragNDropSortListView.this);
                if (child != oldChild) {
                ...
                child = mAdapter.getView(position, null, DragNDropSortListView.this);
    ...
    public interface FloatViewManager {
         ...
         * and the offset provided in {@link DragNDropSortListView#startDrag}.
     ...
     * {@link DragNDropSortListView.DragScrollProfile}.
     * 
     * @param ssp
     */
     ...
     * {@link DragNDropSortListView#setDragScrollProfile(DragScrollProfile)} to
     * set custom profile.
     * 
     * @author heycosmo
     *
    ...
    private class DragScroller implements Runnable {
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                DragNDropSortListView.this.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
        ...
        public void run() {
            ...
            DragNDropSortListView.this.layoutChildren();
            invalidate();
